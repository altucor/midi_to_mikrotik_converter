...
class MTrkChunk
{
    public:
        ...
        MTrkChunk();
        virtual ~MTrkChunk();
        int setInitData( int chunk_ptr, vector<unsigned char> chunk_data, int _mtrk_octave_shift, int _mtrk_note_shift );
        ...
        vector<double> getFreqNotes();
        vector<vector<int>> getLengthNotesON();
        vector<vector<int>> getLengthNotesOFF();
        vector<string> getSymbolicNotes();
    ...
    private:
        ...
        // Persists
        int cmd_note_on  = 144;
        int cmd_note_off =  128;
        int cptr = 0;
        vector<unsigned char> cdata;
        int mtrk_octave_shift = 0;
        int mtrk_note_shift = 0;
        int mtrk_start_pos = 0;
        int mtrk_end_pos = 0;
        int mtrk_sz = 0;
        int tempo = 0;
        int track_name_length = 0;
        string track_name = "";
        int track_text_length = 0;
        string track_text = "";
        int note_channel = 0;
        int first_note_on_byte_addr = 0;
        vector<unsigned char> chunk_body;
        vector<vector<unsigned char>> events_note_on;
        vector<vector<unsigned char>> events_note_off;
        vector<double> chunk_freq_notes;
        vector<vector<int>> chunk_length_notes_on;
        vector<vector<int>> chunk_length_notes_off;
        vector<string> chunk_symbolic_notes;
        vector<int> first_delay;
        vector<string> symbolic_notes = {   "C 0", "C# 0", "D 0", "Eb 0", "E 0", "F 0", "F# 0", "G 0", "G# 0", "A 0", "Bb 0", "B 0",  /* #0 */
        ...
        vector<double> freq_notes = {   8.18,    8.66,    9.18,    9.72,    10.30,   10.91,     11.56,    12.25,   12.98,   13.75,  14.57,   15.43,    /* #0 */
